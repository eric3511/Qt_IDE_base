...
void IntegersAspect::setValue(const QList<int> &value)
{
    // 使用 lambda 表达式，明确告诉编译器：把每个 int 转换成 QVariant
    BaseAspect::setValue(transform(value, [](int v) { return QVariant::fromValue(v); }));
}

void IntegersAspect::setDefaultValue(const QList<int> &value)
{
    // 同理，使用 lambda 表达式
    BaseAspect::setDefaultValue(transform(value, [](int v) { return QVariant::fromValue(v); }));
}
